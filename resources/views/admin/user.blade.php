...
@section('title', 'Users | Admin Govind')
    ...
    >
        ...
        <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-[11px] text-[#285E5E]/80 leading-tight">
                    Total: <span class="font-semibold text-[#1E4B4B]">{{ $users->total() ?? $users->count() }}</span> registered users
                </p>
        ...
        <div class="bg-white/70 backdrop-blur-xl border border-[#C1DCDC]/60 rounded-2xl shadow-[0_12px_30px_rgba(0,0,0,0.06)]">
            ...
            <div class="px-4 py-3 border-b border-[#C1DCDC]/60 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div class="text-[11px] text-[#285E5E]/80 leading-tight">
                    Showing
                    <span class="font-semibold text-[#1E4B4B]">{{ $users->firstItem() ?? 1 }} - {{ $users->lastItem() ?? $users->count() }}</span>
                    of
                    <span class="font-semibold text-[#1E4B4B]">{{ $users->total() ?? $users->count() }}</span>
                    users
                </div>
                        ...
                        @forelse ($users as $index => $user)
                            ...
                            <tr class="hover:bg-[#F3FBFB] transition-colors">
                                ...
                                <td class="px-4 py-3 align-top">
                                    <div class="leading-tight text-[#285E5E]">{{ $user->email }}</div>
                                    <div class="text-[11px] text-[#285E5E]/70 leading-tight">
            ...
            @if(method_exists($users, 'links'))
                <div class="px-4 py-3 border-t border-[#C1DCDC]/60 flex items-center justify-between text:[12px] text-[#285E5E]/90">
                    <div>
                        Showing
                        <span class="font-semibold text-[#1E4B4B]">{{ $users->firstItem() }}</span>
                        -
                        <span class="font-semibold text-[#1E4B4B]">{{ $users->lastItem() }}</span>
                        of
                        <span class="font-semibold text-[#1E4B4B]">{{ $users->total() }}</span>
                        users
                    </div>
        ...
        <div x-show="showCreate" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display: none;">
            ...
            <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-xl border border-[#C1DCDC]/80">
                ...
                <form action="{{ route('admin.user.store') }}" method="POST" enctype="multipart/form-data" class="px-5 py-4 space-y-4 text-[13px]">
                    @csrf
            ...
            return {
                ...
                init(payload) {
                    ...
                    @if ($errors->any() && old('_from') === 'edit')
                        this.editUser.id        = {{ old('id') ?? 'null' }};
                        this.editUser.name      = "{{ old('name') }}";
                        this.editUser.email     = "{{ old('email') }}";
                        this.editUser.phone     = "{{ old('phone') }}";
                        this.editUser.address   = "{{ old('address') }}";
                        this.editUser.role      = "{{ old('role') }}";
                        this.showEdit = true;
                    @endif
                },
                ...
                editActionUrl() {
                    return "{{ route('admin.user.update', ['id' => '__ID__']) }}"
