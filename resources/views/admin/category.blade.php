...
@section('title', 'Category | Admin Govind')
@section('page_title', 'Program Categories')
@section('page_subtitle', 'Manage the program category list')
...
@section('content')
    ...
    >
        ...
        <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-[11px] text-[#417F7F] leading-tight">
                    Total: {{ $categories->total() ?? $categories->count() }} registered categories
                </p>
            </div>
        ...
        <div class="bg-white/60 backdrop-blur-xl border border-[#A3D4D4] rounded-2xl shadow-sm">
                        ...
                        @forelse ($categories as $index => $category)
                            <tr class="hover:bg-[#E3F3F3]">
                                ...
                                <td class="px-4 py-3 font-medium text-[#285E5E]">
                                    <div class="leading-tight">{{ $category->name }}</div>
            ...
            @if(method_exists($categories, 'links'))
                <div class="px-4 py-3 border-t border-[#A3D4D4] flex items-center justify-between text-[12px] text-[#417F7F]">
                    <div>
                        Showing
                        <span class="font-semibold text-[#285E5E]">{{ $categories->firstItem() }}</span>
                        -
                        <span class="font-semibold text-[#285E5E]">{{ $categories->lastItem() }}</span>
                        of
                        <span class="font-semibold text-[#285E5E]">{{ $categories->total() }}</span>
                        categories
                    </div>
                    <div>
        ...
        <div x-show="showCreate" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display:none;">
            ...
            <div class="relative w-full max-w-lg bg-white/90 rounded-2xl shadow-xl border border-[#A3D4D4]">
                ...
                <form action="{{ route('admin.category.store') }}" method="POST" class="px-5 py-4 space-y-4 text-[13px]">
                    @csrf
    ...
    function categoryPage() {
        return {
            ...
            editActionUrl() {
                return "{{ route('admin.category.update', ['id' => '__ID__']) }}"
