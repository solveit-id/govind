...
@section('title', 'Programs | Admin Govind')
@section('page_title', 'Programs')
@section('page_subtitle', 'Manage certification programs')
    ...
    >
        ...
        @if (session('success'))
            ...
            >
                <div class="flex-1">
                    ...
                    <div class="text-[12px] leading-tight">{{ session('success') }}</div>
                </div>
        ...
        @if (session('error'))
            ...
            >
                <div class="flex-1">
                    ...
                    <div class="text-[12px] leading-tight">{{ session('error') }}</div>
                </div>
        ...
        <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-[11px] text-[#5A9A9A] leading-tight">
                    Total: {{ $programs->total() ?? $programs->count() }} registered programs
                </p>

                @if(request('q'))
                    <div class="text-[11px] text-[#5A9A9A] leading-tight mt-1">
                        Search results for:
                        <span class="font-semibold text-[#285E5E]">"{{ request('q') }}"</span>
                    </div>
                @endif
            </div>
        ...
        <div class="bg-white/60 backdrop-blur-xl border border-[#A3D4D4] rounded-2xl shadow-sm">
            ...
            <div class="px-4 py-3 border-b border-[#A3D4D4] flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div class="text-[11px] text-[#5A9A9A] leading-tight">
                    Showing
                    <span class="font-semibold text-[#285E5E]">{{ $programs->firstItem() ?? 1 }} - {{ $programs->lastItem() ?? $programs->count() }}</span>
                    of
                    <span class="font-semibold text-[#285E5E]">{{ $programs->total() ?? $programs->count() }}</span>
                    programs
                </div>

                <form action="{{ route('admin.program') }}" method="GET" class="flex items-center gap-2">
                    <div class="relative">
                        <input
                            type="text"
                            name="q"
                                    ...
                                    <div class="flex items-start justify-center gap-2">
                                        <button
                                            ...
                                            @click="openEditModal({
                                                ...
                                                slug: @js($program->slug),
                                                category_id: {{ $program->category_id }},
                                                duration: @js($program->duration),
                                                price: '{{ $program->price }}',
            ...
            @if(method_exists($programs, 'links'))
                <div class="px-4 py-3 border-t border-[#A3D4D4] flex items-center justify-between text-[12px] text-[#417F7F]">
                    <div>
                        Showing
                        <span class="font-semibold text-[#285E5E]">{{ $programs->firstItem() }}</span>
                        -
                        <span class="font-semibold text-[#285E5E]">{{ $programs->lastItem() }}</span>
                        of
                        <span class="font-semibold text-[#285E5E]">{{ $programs->total() }}</span>
                        programs
                    </div>
        ...
        <div x-show="showCreate" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display:none;">
            ...
            <div class="relative w-full max-w-2xl bg-white/95 rounded-2xl shadow-xl border border-[#A3D4D4]">
                ...
                <form action="{{ route('admin.program.store') }}" method="POST" enctype="multipart/form-data" class="px-5 py-4 space-y-4 text-[13px]">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="text" name="duration" value="{{ old('duration') }}" placeholder="Example: 3 Months"
                                class="w-full rounded-xl border border-[#A3D4D4] px-3 py-2 text-[13px] text-[#285E5E] bg-white
                                    focus:outline-none focus:ring-2 focus:ring-[#5AAEAE] focus:border-[#5AAEAE]" />
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <textarea name="short_desc" rows="2"
                            class="w-full rounded-xl border border-[#A3D4D4] px-3 py-2 text-[13px] text-[#285E5E] bg-white
                                focus:outline-none focus:ring-2 focus:ring-[#5AAEAE] focus:border-[#5AAEAE]"
                            placeholder="Short program description... 😊">{{ old('short_desc') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="long_desc" rows="4"
                            class="w-full rounded-xl border border-[#A3D4D4] px-3 py-2 text-[13px] text-[#285E5E] bg-white
                                focus:outline-none focus:ring-2 focus:ring-[#5AAEAE] focus:border-[#5AAEAE]"
                            placeholder="Full program explanation, benefits, modules, etc.">{{ old('long_desc') }}</textarea>
                    </div>
    ...
    <script>
        function programPage() {
            return {
                ...
                editActionUrl() {
                    return "{{ route('admin.program.update', ['id' => '__ID__']) }}"
